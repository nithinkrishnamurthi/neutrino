...
impl AsgiManager {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("  Path prefix: {}", self.config.path_prefix);

        // Build uvicorn command
        // Assumes uvicorn_app.py was generated by the CLI
        let mut cmd = Command::new("uvicorn");
        cmd.arg("uvicorn_app:app")
            .arg("--host")
            .arg("127.0.0.1")
            .arg("--port")
    ...
    async fn wait_for_ready(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for attempt in 1..=max_attempts {
            // Try to connect to the ASGI app
            ...
            match reqwest::get(&url).await {
                ...
                Ok(response) => {
                    warn!(
                        "ASGI application returned status {} (attempt {})",
                        response.status(),
                        attempt
                    );
                }
